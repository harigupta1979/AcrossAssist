...
export class PermissionsMasterComponent {
  ...
  toggleitemSelect(item: any) {
    ...
    const parentGroup = this.menuList.find(group => 
      group.type === 'group' && group.children?.some((child: { id: any; })=> child.id === item.id)
    );
  
    if (parentGroup) {
      // If any child is selected, select the parent group
      parentGroup.isSelect = parentGroup.children.some((child: { isSelect: any; }) => child.isSelect);
    }
  }
  ...
  let data:any = await this.service.PostRoleMenuMappinging(obj);
    ...
    else if ((data['FinalMode'] == 'INSERT' || data['FinalMode'] == 'UPDATE') && data['Message'] != null) {
